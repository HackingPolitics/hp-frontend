...
export class HocuspocusProvider extends EventEmitter {
  ...
  constructor(options: Partial<HocuspocusProviderOptions> = {}) {
    ...
    this.options.awareness = options.awareness
      ? options.awareness
      : new Awareness(this.document)
    this.options.WebSocketPolyfill = options.WebSocketPolyfill
      ? options.WebSocketPolyfill
      : WebSocket
    this.shouldConnect =
      options.connect !== undefined ? options.connect : this.shouldConnect
  ...
  checkConnection() {
    ...
    if (
      this.options.messageReconnectTimeout >=
  ...
  onClose(event: CloseEvent) {
    ...
    if (this.shouldConnect) {
      const wait = math.round(
        math.min(
          math.log10(this.failedConnectionAttempts + 1) *
          this.options.reconnectTimeoutBase,
  ...
  subscribeToBroadcastChannel() {
    if (!this.subscribedToBroadcastChannel) {
      bc.subscribe(
        ...
        this.broadcastChannelSubscriber.bind(this)
      )
